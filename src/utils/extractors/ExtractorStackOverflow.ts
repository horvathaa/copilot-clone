...
export default class ExtractorStackOverflow extends ExtractorAbstract {
    ...
    extractSnippets = (options: FetchPageResult): SnippetResult[] => {
        ...
        const answersWithCodeBlock = Array.from(
            target.window.document.querySelectorAll('.answer')
            ...
            .map(
                (item: any) =>
                    ({
                        ...
                        votes: parseInt(
                            item.querySelector('.js-vote-count').textContent
                        ),
                        ...
                        sourceURL: `https://${this.URL}${
                            item.querySelector('.js-share-link').href
                        }`,
                        hasCheckMark:
                            item.querySelector('iconCheckmarkLg') != null,
                    } as SnippetResult)
            )
